<form action="{{ $action }}" method="POST" class="space-y-6">
    ...
    <div>
        ...
        <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border px-3 py-2 @error('status') border-red-500 @enderror" required>
            ...
            <option value="{{ \App\Enums\StatusPelatihan::Dibuka->value }}" {{ old('status', $pelatihan?->status?->value) === \App\Enums\StatusPelatihan::Dibuka->value ? 'selected' : '' }}>Dibuka</option>
            <option value="{{ \App\Enums\StatusPelatihan::Ditutup->value }}" {{ old('status', $pelatihan?->status?->value) === \App\Enums\StatusPelatihan::Ditutup->value ? 'selected' : '' }}>Ditutup</option>
            <option value="{{ \App\Enums\StatusPelatihan::Selesai->value }}" {{ old('status', $pelatihan?->status?->value) === \App\Enums\StatusPelatihan::Selesai->value ? 'selected' : '' }}>Selesai</option>
        </select>
        @error('status')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
